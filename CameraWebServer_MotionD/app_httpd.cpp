...
// Ensure valid settings based on available PSRAM
if (psramFound()) {
    ...
    Serial.println("Initializing camera for motion detection...");
    delay(500);  //  Add delay before initializing the camera


    //  Prevent re-installing the ISR service if it's already running
   static bool is_isr_installed = false;
    if (!is_isr_installed) {
    gpio_install_isr_service(0);
    is_isr_installed = true;
    } else {
    Serial.println("GPIO ISR service already installed, skipping...");
    }
    ...
    // Setup correct pin configuration for AI Thinker ESP32-CAM
    camera.pinout.aithinker();
    camera.brownout.disable();
    camera.resolution.vga();  // Start with VGA for motion detection
    camera.quality.high();
    ...
    //  Initialize camera only if it hasn't been initialized already
...
if (!camera_initialized) {
    ...
    Serial.println("Camera OK. Awaiting motion...");
}


void save_image_to_sd(uint8_t *buffer, size_t len) {
    if (!SD_MMC.begin()) {
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    // Set HTTP response headers for streaming
    ...
    while (true) {
        ...
        // If motion is detected, capture in high resolution
        if (detection.triggered()) {
            Serial.println("Motion detected! Capturing high-resolution image...");

            camera.resolution.at(FRAMESIZE_UXGA, []() { // Switch to UXGA
                Serial.println("Switched to UXGA resolution");

                camera_fb_t *fb = esp_camera_fb_get();
                if (!fb) {
                    Serial.println("Failed to capture high-resolution image.");
                } else {
                    save_image_to_sd(fb->buf, fb->len);
                    esp_camera_fb_return(fb);
                    Serial.println("Saved high-resolution image to SD card");
                }
              });


            Serial.println("Switching back to VGA...");
        }

        // Stream the low-resolution image
        if (fb->format == PIXFORMAT_JPEG) {
            _jpg_buf = fb->buf;
            _jpg_buf_len = fb->len;
        } else {
