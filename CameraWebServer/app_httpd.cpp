...
camera_fb_t* do_capture() {
#if CONFIG_LED_ILLUMINATOR_ENABLED
    enable_led(true);
    vTaskDelay(150 / portTICK_PERIOD_MS);  // LED delay for proper illumination
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    // Send JPEG frame (using existing chunked encoding function)
    ...
    log_i("Capture: %uB", jchunk.len);
    return ESP_OK;
}
// In app_httpd.cpp (or a shared header)
camera_fb_t *latest_fb = NULL;
xSemaphoreHandle fb_mutex = xSemaphoreCreateMutex();
void motion_detection_task(void *pvParameters) {
    for (;;) {
        // Capture a frame for motion analysis
        camera_fb_t* fb = do_capture();
        if (!fb) {
        ...
        if (motion_detected) {
            ...
            // Save the captured frame to the global buffer.
            // Use a mutex to protect the global pointer.
            ...
            // If no motion is detected, release the frame immediately.
            esp_camera_fb_return(fb);
        }
        // Delay before the next capture (adjust based on performance needs)
...
static esp_err_t latest_handler(httpd_req_t *req) {
    ...
    // Send JPEG image
    httpd_resp_set_type(req, "image/jpeg");
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=latest.jpg");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");

    char ts[32];
    snprintf(ts, 32, "%ld.%06ld", latest_fb->timestamp.tv_sec, latest_fb->timestamp.tv_usec);
    httpd_resp_set_hdr(req, "X-Timestamp", (const char *)ts);

    jpg_chunking_t jchunk = { req, 0 };
    bool jpeg_converted = frame2jpg_cb(latest_fb, 80, jpg_encode_stream, &jchunk);
    xSemaphoreGive(fb_mutex);
    if (!jpeg_converted) {
        log_e("Latest JPEG conversion failed");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
    log_i("Latest image: %uB", jchunk.len);
    return ESP_OK;
}
...
void triggerCapture() {
    camera_fb_t *fb = do_capture();
    if (fb) {
        // Optionally, process or save the frame here.
        esp_camera_fb_return(fb);
    }
}
